...
SERVICE_NAME="$(jq -r '.grafana_proxy_service_name' config.json)"
SERVICE_NAMESPACE="$(jq -r '.grafana_namespace' config.json)"
SERVICE_PORT="$(jq -r '.grafana_proxy_service_port' config.json)"
LISTEN_PORT="$(jq -r '.grafana_proxy_listen_port' config.json)"
USERNAME="$(jq -r '.grafana_username' config.json)"
PASSWORD="$(jq -r '.grafana_password' config.json)"
OS="$(uname)"
...
if [ "${OS}" == 'Linux' ]; then
  echo "Grafana UI on: http://${HOST_IP}:${LISTEN_PORT} w/ ${USERNAME}/${PASSWORD}"
fi
