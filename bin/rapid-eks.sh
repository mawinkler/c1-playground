...
function main() {
  ...
  export CLUSTER_NAME=$(jq -r '.cluster_name' $PGPATH/config.json)-$(openssl rand -hex 4)
...
for NAMESPACE in \$(cat $PGPATH/config.json | jq -r '.services[].namespace' | sort | uniq); do
