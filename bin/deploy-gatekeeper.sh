...
function create_tls_secret() {
  ...
  envsubst <$PGPATH/templates/gatekeeper-server-tls.conf >gatekeeper/webhook-server-tls.conf
...
function deploy_gatekeeper() {
  printf '%s\n' "deploy gatekeeper"
  envsubst <$PGPATH/templates/gatekeeper-overrides.yaml >$PGPATH/overrides/gatekeeper-overrides.yaml

  helm repo add gatekeeper https://open-policy-agent.github.io/gatekeeper/charts
  helm upgrade \
    gatekeeper \
    --values $PGPATH/overrides/gatekeeper-overrides.yaml \
    --namespace ${NAMESPACE} \
    --install \
