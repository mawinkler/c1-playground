...
function main() {
  ...
  export CLUSTER_NAME=$(yq '.cluster_name' $PGPATH/config.yaml | tr '[:upper:]' '[:lower:]')-f-$(openssl rand -hex 4)
...
EOF
  ...
  cat <<EOF >$PGPATH/bin/rapid-eks-down-fargate.sh
set -e
...
printf '\n%s\n' "###TASK-COMPLETED###"
EOF
  chmod +x $PGPATH/bin/rapid-eks-down-fargate.sh
  echo "Run rapid-eks-down-fargate.sh to tear down everything"
}

function cleanup() {
  $PGPATH/bin/rapid-eks-down-fargate.sh
  if [ $? -eq 0 ] ; then
    return
  fi
  false
