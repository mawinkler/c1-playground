...
REG_USERNAME="$(jq -r '.services[] | select(.name=="harbor") | .reg_username' config.json)"
REG_PASSWORD="$(jq -r '.services[] | select(.name=="harbor") | .reg_password' config.json)"
REG_HTPASSWD="$(jq -r '.services[] | select(.name=="harbor") | .reg_htpasswd' config.json)"
...
function deploy_harbor() {
  ...
  SERVICE_TYPE=${SERVICE_TYPE} \
    ...
    envsubst <templates/harbor-overrides.yaml >overrides/harbor-overrides.yaml

  # Install harbor
  helm -n ${NAMESPACE} upgrade \
    harbor \
    --install \
    --values=overrides/harbor-overrides.yaml \
    harbor/harbor
}
...
function create_ingress() {
  ...
  SERVICE_NAME=${SERVICE_NAME} \
    ...
    HOSTNAME=${HOSTNAME} \
    LISTEN_PORT=${LISTEN_PORT} \
    envsubst <templates/harbor-ingress.yaml | kubectl apply -f - -o yaml
...
function main() {
  ...
  if is_linux ; then
    ...
    if is_kind ; then
      ./deploy-proxy.sh harbor
      echo "harbor UI on: https://$(hostname -I | awk '{print $1}'):${LISTEN_PORT}" | tee -a services
    fi
  fi
  if is_darwin ; then
    create_ingress
